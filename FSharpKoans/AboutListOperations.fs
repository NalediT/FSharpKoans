    ...
    *)
    ...
    let ``03 Reversing a list, the hard way`` () =
        let rev (xs : 'a list) : 'a list =
           let rec reverse accumulator = 
                function 
                |[] -> accumulator
                |a :: xs  -> reverse (a :: accumulator) xs
    ...
    let ``05 Fixed-function mapping, the hard way (part 1).`` () =
        let map (xs : int list) : int list =
            List.map(fun k -> k + 1) xs // write a function which adds 1 to each element
    ...
    let ``06 Fixed-function mapping, the hard way (part 2).`` () =
        let map (xs : int list) : int list =
            List.map (fun k -> k * 2) xs // write a function which doubles each element
   ...
   *)
    ...
    let ``07 Specified-function mapping, the hard way`` () =
        let map (f : 'a -> 'b) (xs : 'a list) : 'b list =
            let rec maplist f acc =
                match acc with 
                |[] -> []
                |a::b -> (f a) :: maplist f b 
            match xs with
            |[] -> []
            |_ -> maplist f xs // write a map which applies f to each element
    ...
    let ``09 Specified-function filtering, the hard way`` () =
        let filter (f : 'a -> bool) (xs : 'a list) : 'a list =
            let rec filterlist f list = 
                match list with
                |a::b -> match (f a) with
                    |true -> a:: filterlist f b 
                    |false -> filterlist f b
                |_ -> []
            match xs with 
            |[] -> []
            |_ -> filterlist f xs// write a function which filters based on the specified criteria
...
*)
    ...
    let ``14 A fold which sums a list`` () =
        let fold initialState xs =
          List.fold (fun sum (k)->sum+k) initialState xs   // write a function to do what's described above
    ...
    let ``15 A fold which multiplies a list`` () =
        let fold initialState xs =
             List.fold (fun multiply (k)->multiply*k) initialState xs  // write a function to multiply the elements of a list
