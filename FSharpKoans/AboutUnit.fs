    ...
    let ``07 Partially specifying arguments (Part 3).`` () =
        ...
        let cows = 
            fun x->
                let a = f "cow" x 
                let b = ", de gozaru"
                let c=sprintf "%s%s" a b
                c
            // <-- multiple words on this line, or you may want to make this a multi-line thing.  You MUST use `f`.// <-- multiple words on this line, or you may want to make this a multi-line thing.  You MUST use `f`.
